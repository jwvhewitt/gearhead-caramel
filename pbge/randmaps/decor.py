    ...
    def __call__(self, gb, room, archi):
        ...
        for x in range(room.area.x, room.area.x + room.area.width - 1):
            for y in range(room.area.y, room.area.y + room.area.height - 1):
                pos = (x, y)
                if room.is_basic_wall(gb, x, y) and room.is_good_spot_for_wall_decor(gb, pos, archi):
                    good_wall_spots.append(pos)
                elif x > 0 and y > 0 and \
                        not gb.tile_blocks_movement(x, y, archi.mmode) and not gb.get_wall(x, y) \
                        and not gb.get_decor(x, y) and gb.wall_wont_block(x, y, archi.mmode):
        ...
        if self.WALL_DECOR:
            for t in range(max(int(len(good_wall_spots) * self.WALL_FILL_FACTOR), random.randint(1, 6))):
                ...
                if room.is_good_spot_for_wall_decor(gb, (x, y), archi):
