...
class OddJobsBiotechnology(Plot):
    ...
    def NPC_offers(self, camp: gears.GearHeadCampaign):
        ...
        if camp.party_has_skill(gears.stats.Biotechnology):
            if self._rumor_memo_delivered:
                mylist.append(Offer(
                    "[You_heard_right]. We've discovered samples of an unknown {MATERIAL} but have been unable to determine its structure. Would you mind providing an expert opinion about it?".format(**self.elements),
                ...
                mylist.append(Offer(
                    "[HELLO] I've been trying to analyze an unknown {MATERIAL}, but I suspect it's of biotechnological origin. Would you be willing to examine it for me?".format(**self.elements),
    ...
    def _attempt_analysis(self, camp: gears.GearHeadCampaign):
        ...
        if skroll > gears.stats.get_skill_target(self.rank, gears.stats.DIFFICULTY_AVERAGE):
            if truey_or_fake:
                pbge.alert("You determine that the {MATERIAL} is definitely biotechnological; it probably originates from PreZero times.".format(**self.elements))
                ghcutscene.SimpleMonologueDisplay("[THATS_INTERESTING] Here is a reward for helping with my research.", npc)(camp)
            else:
                pbge.alert("You determine that the {MATERIAL} is of natural origin. {NPC} probably just made some measurement errors.".format(**self.elements))
                ghcutscene.SimpleMonologueDisplay("Well that's disappointing. Still, here's a reward for helping with my research.", npc)(camp)
            ...
            pbge.alert("After much analysis, you conclude that you don't know what this {MATERIAL} is or where it came from.".format(**self.elements))
            ghcutscene.SimpleMonologueDisplay("Thanks for trying, at least. I guess I'm going to have to figure this out on my own.", npc)(camp)
            relationship = camp.get_relationship(npc)
...
class OddJobsPerformance(Plot):
    ...
    def custom_init( self, nart ):
        npc = self.seek_element(nart, "NPC", self._is_good_npc, must_find=True, lock=True,
                                scope=self.elements["METROSCENE"])
        self.elements["NPC_SCENE"] = npc.scene
        self.reward = self.register_element("_REWARD", gears.selector.calc_mission_reward(self.rank, 50))
        self.expiration = TimeExpiration(nart.camp, time_limit=5)
        set_npc_recharge(npc, nart.camp)
        return True
    ...
    def NPC_offers(self, camp: gears.GearHeadCampaign):
        ...
        if self._rumor_memo_delivered and camp.party_has_skill(gears.stats.Performance):
            mylist.append(Offer(
                "[You_heard_right]. I'm looking for someone to play music at {NPC_SCENE}; pay is ${_REWARD:,}.".format(**self.elements),
            ...
            mylist.append(Offer(
                "[HELLO] I'm looking for a musician to play at {NPC_SCENE}; [You_have_the_needed_skills]. Pay is ${_REWARD:,}.".format(**self.elements),
    ...
    def _play_concert(self, camp: gears.GearHeadCampaign):
        ...
        if skroll > gears.stats.get_skill_target(self.rank, gears.stats.DIFFICULTY_LEGENDARY):
            ...
            ghcutscene.SimpleMonologueDisplay("[THAT_WAS_INCREDIBLE] Here, you attracted so many people that I can pay twice what we agreed to.", npc)(camp)
            ...
            ghcutscene.SimpleMonologueDisplay("[THAT_WAS_THE_WORST] I think you actually drove away some people... You have failed at this job.", npc)(camp)
            relationship = camp.get_relationship(npc)
            relationship.reaction_mod -= random.randint(1, 10)
...
class OddJobsRepair(Plot):
    ...
    def NPC_offers(self, camp: gears.GearHeadCampaign):
        ...
        if self._rumor_memo_delivered and camp.party_has_skill(gears.stats.Repair):
            mylist.append(Offer(
                "[You_heard_right]. Would you mind looking at it for me? I can pay ${_REWARD:,} if you fix it.".format(**self.elements),
