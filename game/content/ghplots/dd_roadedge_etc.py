...
class BlackMarketBluesMain(Plot):
    ...
    def MISSION_GATE_menu(self, camp, thingmenu):
        if self.black_market_found:
            thingmenu.add_item("Go to the Black Market.", self._go_to_market, data=camp)
        if self.bandit_base_found and not self.bandit_base_destroyed:
            thingmenu.add_item("Attack the {} base.".format(self.elements["FACTION"]), self._attack_the_enemy_base, data=camp)

    def SKULL_SIGN_menu(self, camp, thingmenu):
        thingmenu.add_item("[CONTINUE]", self._read_the_sign, data=camp)

    def _read_the_sign(self, wid, _ev):
        camp = wid.data
    ...
    def _go_to_market(self, wid, _ev):
        camp = wid.data
        if self.black_market_entry_status == self.BMES_ENTERED:
            ...
            self._attack_the_black_market_directly(camp)
        else:
            self._attempt_entry_to_black_market(camp)

    def _attack_the_black_market(self, wid, _ev):
        self._attack_the_black_market_directly(wid.data)

    def _attack_the_black_market_directly(self, camp):
    ...
    def _attack_the_enemy_base(self, wid, _ev):
        camp = wid.data
        my_mission = missionbuilder.BuildAMissionSeed(
    ...
    def _enter_the_black_market(self, wid, _ev):
        camp = wid.data
    ...
    def _win_entry_fight(self, camp):
        mymenu = pbge.widgetmenu.AlertMenuWidget("How do you want to deal with the {}?".format(self.elements["BLACK_MARKET"]))
        _=mymenu.add_item("Attack and destroy it.", self._attack_the_black_market, data=camp)
        _=mymenu.add_item("Enter on foot", self._enter_the_black_market, data=camp)
...
class MechaGraveyardAdventure(Plot):
    ...
    def COMPY_menu(self, camp: gears.GearHeadCampaign, thingmenu: pbge.widgetmenu.MenuWidget):
        if self.zombots_active:
            thingmenu.desc = "{} The lights are blinking wildly.".format(thingmenu.desc)
            _=thingmenu.add_item("Smash the system", self._smash, data=camp)
            if self.got_shutdown:
                _=thingmenu.add_item("Activate the FINAL_DEATH command", self._shutdown, data=camp)

    def _smash(self, wid, _ev):
        camp = wid.data
    ...
    def _shutdown(self, wid, _ev):
        ...
        self.subplots["OUTENCOUNTER"].end_plot(camp)
        camp.check_trigger("WIN", self)
        _=BiotechDiscovery(camp, "I found a PreZero lab where they were developing self-repair technology",
    ...
    def LORE3_menu(self, camp, thingmenu):
        if self.alpha_full:
            thingmenu.add_item("Drain Alpha Chamber", self._toggle_alpha, data=camp)
        else:
            thingmenu.add_item("Fill Alpha Chamber", self._toggle_alpha, data=camp)
        if self.beta_full:
            thingmenu.add_item("Drain Beta Chamber", self._toggle_beta)
        else:
            thingmenu.add_item("Fill Beta Chamber", self._toggle_beta)

    def _toggle_alpha(self, wid, _ev):
        camp = wid.data
        if self.alpha_full:
            ...
            _=pbge.alerts.TextAlert("ERROR: Alpha Chamber not responding. Please consult manual.")

    def _toggle_beta(self, _wid, _ev):
        if self.beta_full:
    ...
    def BIO_A_menu(self, _camp, thingmenu):
        if self.alpha_full:
    ...
    def BIO_B_menu(self, camp, thingmenu):
        if self.set_collector:
            if self.beta_full:
                ...
                thingmenu.add_item("Get the box", self._get_box, data=camp)
        else:
            if self.beta_full:
    ...
    def _get_box(self, wid, _ev):
        camp = wid.data
        _=pbge.alerts.TextAlert("You retrieve the box from the biotank. It appears to be some kind of mecha component.")
        self.set_collector = False
        myitem = gears.selector.get_design_by_full_name("NC-1 Self Repair System")
        camp.give_item(myitem)
    ...
    def LORE2_menu(self, camp, thingmenu):
        ...
        thingmenu.add_item("Search for hidden files", self._search_l2, data=camp)

    def _read_overview(self, _wid, _ev):
        _=pbge.alerts.TextAlert(
    ...
    def _search_l2(self, wid, _ev):
        ...
        if pc:
            if pc is camp.pc:
                _=pbge.alerts.TextAlert("You easily hack into the ancient computer system.")
            else:
                _=pbge.alerts.TextAlert("{} hacks into the ancient computer system.".format(pc))
            _=pbge.alerts.TextAlert("You discover an additional data file for a secondary project.")
            _=pbge.alerts.TextAlert("Test subject HC-Alpha shows the potential for the NC-1 system to be used not just for battlemovers, but individual soldiers as well. Biotechnology allows perfect fusion of organic and inorganic components.")
            _=pbge.alerts.TextAlert("If the neural degradation problem can be solved, imagine the potential: Immortal soldiers who instantly recover from any damage. The keys to their immortality safely held by illumos who control the power source.")
            _=pbge.alerts.TextAlert("Even if the degradation problem is intractable, there are ways to weaponize this phenomenon. Imagine high-C munitions loaded with NC bionites. Initial casulties could be reanimated to eliminate surviving defenders.")
            _=pbge.alerts.TextAlert("Anyway, it's something to think about when we submit the third round of grant applications.")
            self.got_shutdown = True
        else:
            _=pbge.alerts.TextAlert("This computer uses a PreZero operating system that is far beyond your understanding.")
    ...
    def LORE1_menu(self, camp, thingmenu):
        ...
        thingmenu.add_item("Search for hidden files", self._search_l1, data=camp)

    def _read_hc(self, _wid, _ev):
        _=pbge.alerts.TextAlert(
    ...
    def _search_l1(self, wid, _ev):
        ...
        if pc:
            if pc is camp.pc:
                ...
                _=pbge.alerts.TextAlert("{} hacks into the ancient computer system.".format(pc))
            _=pbge.alerts.TextAlert("You discover an additional log file from Dr. Millicent Savini which someone attempted to delete from the database.")
            _=pbge.alerts.TextAlert("\"Attempts to restrain the NC-1 bionite seem to be interpreted by the gestalt intelligence as a form of damage. Thus far, it has outmaneuvered every security protocol we have attempted.\"")
            _=pbge.alerts.TextAlert("\"Herbert's condition is deteriorating as more of his biomass is being replaced by machinery. It is not clear what effect the infection has had on his brain. In any case, I have restrained him in a specimen vat and will be conducting further tests.\"")
            _=pbge.alerts.TextAlert("\"I have installed an emergency shutdown switch into the factory control mainframe. In the event that we lose control of NC-1, the command FINAL_DEATH will cut power to the system. Active bionites will be moved to the containment system.\"")
            self.got_shutdown = True
        else:
            _=pbge.alerts.TextAlert("This computer uses a PreZero operating system that is far beyond your understanding.")
