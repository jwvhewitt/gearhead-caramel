...
class DZREPR_BasePlot(Plot):
    ...
    def t_UPDATE(self, camp):
        if self.start_missions:
            if (self.MISSION_IDENTS[0] not in self.subplots or not self.subplots[
                self.MISSION_IDENTS[0]]) and not self.final_loaded:
    ...
    def load_missions(self, camp):
        if self.elements[E_MISSION_NUMBER] >= self.NUMBER_OF_MISSIONS_BEFORE_CONCLUSION:
            init = pbge.plots.PlotState(
                rank=max(self.rank, camp.renown - 3) + self.elements[E_MISSION_WINS] * 3 + 5).based_on(self)
            ...
            while num_miss < 2 and miss_num < len(self.MISSION_LABELS):
                init = pbge.plots.PlotState(
                    rank=max(self.rank, camp.renown - 3) + self.elements[E_MISSION_WINS] * 3).based_on(self)
                nart = game.content.GHNarrativeRequest(camp, init, self.MISSION_LABELS[miss_num],
                                                       game.content.PLOT_LIST)
                if nart.story:
                    nart.build()
...
class DZREPR_BaseMission(Plot):
    ...
    def matches(cls, pstate):
        ...
        return (all(pstate.elements.get(k, 0) == cls.REQUIRES[k] for k in
                    cls.REQUIRES.keys()) and cls.REQUIRED_FACTAGS.issubset(
            pstate.elements["FACTION"].factags)) or cls.LABEL == "DZRE_TEST"
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and npc.combatant and npc not in camp.party:
            goffs.append(Offer(
                ...
                context=ContextTag((context.CUSTOM,)), effect=self.activate_mission,
                data={"reply": "Do you have any idea where {} have their base?".format(self.elements["FACTION"])},
                no_repeats=True
            ))
        return goffs
...
class DZREPR_ShootThemInTheGun(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate, gears.base.Character) and nart.camp.are_faction_allies(candidate, self.elements[
            "METROSCENE"]) and candidate not in nart.camp.party and not nart.camp.are_faction_allies(candidate,
                                                                                                     self.elements[
                                                                                                         "FACTION"])
...
class DZREPR_HighwayPatrol(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and gears.tags.Politician in npc.get_tags() and npc not in camp.party:
            goffs.append(Offer(
                msg="Since {} showed up, several convoys have gone missing between here and {}. If you patrol the highway you might be able to catch them in the act.".format(
                    self.elements["FACTION"], self.elements["DZ_EDGE"].get_city_link(self.elements["METROSCENE"])),
                context=ContextTag((context.CUSTOM,)), effect=self.activate_mission,
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if gears.tags.Politician not in npc.get_tags() and not self.mission_active:
            mygram["[News]"].append(
                "the people in charge know more about {} than they're letting on.".format(self.elements["FACTION"]))
        return mygram
...
class DZREPR_SeeWhatTheyreMadeOf(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        """Get any offers that could apply to non-element NPCs."""
        goffs = list()
        if not self.mission_active and npc not in camp.party and not camp.are_faction_allies(npc,
                                                                                             self.elements["FACTION"]):
            goffs.append(Offer(
                msg="So far our town has mainly been on the defensive; I say it's time to take the fight to {FACTION}. You should go out there and find out what they're made of.".format(
                    **self.elements),
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if not self.mission_active and npc not in camp.party and not camp.are_faction_allies(npc,
            ...
            if npc.combatant:
                ...
                mygram["[News]"].append(
                    "all the pilots in town are busy dealing with {FACTION}".format(**self.elements))
        return mygram
...
class DZREPR_FaultyIntel(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate, gears.base.Character) and candidate.job.tags.intersection((gears.tags.Police,
                                                                                                gears.tags.Military,
                                                                                                gears.tags.Politician)) and candidate not in nart.camp.party and not nart.camp.are_faction_allies(
            candidate, self.elements["FACTION"])
...
class DZREPR_NoBigDealMaybe(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate, gears.base.Character) and candidate.job and candidate.job.tags.intersection((
                                                                                                                 gears.tags.Police,
                                                                                                                 gears.tags.Military,
                                                                                                                 gears.tags.Politician)) and candidate not in nart.camp.party and not nart.camp.are_faction_allies(
            candidate, self.elements["FACTION"])
...
class DZREPR_MedicineShipment(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate, gears.base.Character) and candidate.job and candidate.job.tags.intersection(
            (gears.tags.Medic,)) and candidate not in nart.camp.party and not nart.camp.are_faction_allies(candidate,
                                                                                                           self.elements[
                                                                                                               "FACTION"])
...
class DZREPR_SeekAndDestroy(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate,
                          gears.base.Character) and candidate.combatant and candidate not in nart.camp.party and not nart.camp.are_faction_allies(
            candidate, self.elements["FACTION"])
...
class DZREPR_NotSoHiddenIzzit(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate,
                          gears.base.Character) and candidate.combatant and candidate not in nart.camp.party and nart.camp.are_faction_allies(
            candidate, self.elements["METROSCENE"]) and not nart.camp.are_faction_allies(candidate,
                                                                                         self.elements["FACTION"])
...
class DZREPR_ThoseAreNotBandits(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and npc.combatant and npc not in camp.party:
            goffs.append(Offer(
                msg="For one thing, they spend at least as much time fighting the {METROSCENE} defense force as they do stealing things.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
...
class DZREPR_TheyHaveUsSurrounded(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and not npc.combatant and npc not in camp.party:
            goffs.append(Offer(
                msg="I've heard they have a hidden base, and are keeping watch over all the traffic that comes into or out of {METROSCENE}. We're essentially at their mercy.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
                data={"subject": str(self.elements["FACTION"])},
                subject="get nervous about {FACTION}".format(**self.elements), no_repeats=True
            ))
        return goffs
...
class DZREPR_JustThwackThem(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and npc not in camp.party:
            goffs.append(Offer(
                msg="Everybody knows that {FACTION} has a camp in a [deadzone_residence] just [direction] of here. I don't understand why we can't simply attack them there.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
                data={"subject": str(self.elements["FACTION"])},
                subject="go and fight {FACTION}".format(**self.elements), no_repeats=True
            ))
        return goffs
...
class DZREPR_TheConflictIntensifies(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate, gears.base.Character) and candidate.job.tags.intersection((gears.tags.Police,
                                                                                                gears.tags.Politician,
                                                                                                gears.tags.Military)) and candidate not in nart.camp.party and not nart.camp.are_faction_allies(
            candidate, self.elements["FACTION"])
...
class DZREPR_BanditAbductions(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and not npc.combatant and npc not in camp.party:
            goffs.append(Offer(
                msg="If you spend any time at all on the highway leading into town, you're certain to receive a distress call from someone being attacked by {FACTION}.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
                data={"subject": "these abductions"}, subject="{FACTION} has been abducting".format(**self.elements),
                no_repeats=True
            ))
        return goffs
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if not npc.combatant and not self.mission_active and npc not in camp.party:
            mygram["[News]"].append(
                "{FACTION} has been abducting travelers coming into or leaving {METROSCENE}".format(**self.elements))
        return mygram
...
class DZREPR_HuntingForSecretBase(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate, gears.base.Character) and candidate.job.tags.intersection((gears.tags.Military,
                                                                                                gears.tags.Politician)) and candidate not in nart.camp.party and not nart.camp.are_faction_allies(
            candidate, self.elements["FACTION"])
...
class DZREPR_HiddenNoLonger(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate, gears.base.Character) and candidate.combatant and nart.camp.are_faction_allies(
            candidate,
            self.elements["METROSCENE"]) and candidate not in nart.camp.party and not nart.camp.are_faction_allies(
            candidate, self.elements["FACTION"])
...
class DZREPR_JustifiedParanoia(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and not npc.combatant and npc not in camp.party:
            goffs.append(Offer(
                msg="I know this is going to sound paranoid, but I don't think they're just here to raid the highways. I've heard they have a huge number of mecha just [direction] of town. Maybe {FACTION} is getting ready to take over.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
                data={"subject": "the weird stuff"},
                subject="{FACTION} has been doing some weird stuff".format(**self.elements), no_repeats=True
            ))
        return goffs
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if not npc.combatant and not self.mission_active and npc not in camp.party:
            mygram["[News]"].append(
                "{FACTION} has been doing some weird stuff near {METROSCENE}".format(**self.elements))
        return mygram
...
class DZREPR_FallingStarSponsorship(DZREPR_NPCMission):
    ...
    def _npc_matches(self, nart, candidate):
        return isinstance(candidate,
                          gears.base.Character) and not candidate.combatant and candidate not in nart.camp.party and not nart.camp.are_faction_allies(
            candidate, self.elements["FACTION"])
...
class DZREPR_AngryAtSponsorship(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and not npc.combatant and npc not in camp.party:
            goffs.append(Offer(
                msg="[THEYAREOURENEMY] They think that just because they're strong, they can do what they like in {METROSCENE}. I say it's time to knock them down a peg.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
...
class DZREPR_FOTIBountyHunt(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and not npc.combatant and npc not in camp.party:
            goffs.append(Offer(
                msg="Whatever they're up to, I'm sure it's dreadful. The leaders of {METROSCENE} have offered a bounty for anyone who can defeat {FACTION}, but I doubt it's going to do any good.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
                data={"subject": "{FACTION}'s plans".format(**self.elements)},
                subject="{FACTION} has in store".format(**self.elements), no_repeats=True
            ))
        return goffs
...
class DZREPR_SleepingGiant(DZREPR_BaseMission):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and not npc.combatant and npc not in camp.party:
            goffs.append(Offer(
                msg="I know that every deadzone town has a legend like this, but our elders used to say that there's something extremely dangerous locked down under there. I hate to think what would happen if {FACTION} find it.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
                data={"subject": "the undercity".format(**self.elements)},
                subject="the {METROSCENE} undercity".format(**self.elements), no_repeats=True
            ))
        return goffs
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if not npc.combatant and not self.mission_active and npc not in camp.party:
            mygram["[News]"].append(
                "they say {FACTION} has been searching the {METROSCENE} undercity".format(**self.elements))
        return mygram
...
class DZREPRC_CallOutBattle(DZREPRC_ConclusionTemplate):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and npc not in camp.party:
            goffs.append(Offer(
                msg="You've caused a lot of trouble for {FACTION}, so they've mobilized all of their forces in {METROSCENE} to finish you off once and for all.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if not self.mission_active and npc not in camp.party:
            ...
            mygram["[HELLO_AGAIN]"].append(
                "What are you doing here, [audience]?! Didn't you hear that {FACTION} have challenged you to one final battle?".format(
                    **self.elements))
            mygram["[HELLO_FIRST]"].append(
                "You're [audience], aren't you? [chat_lead_in] {FACTION} have challenged you to one final battle.".format(
                    **self.elements)
            )
        return mygram
...
class DZREPC_CallOutChampionFight(DZREPRC_ConclusionTemplate):
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.mission_active and npc not in camp.party:
            goffs.append(Offer(
                msg="You've caused a lot of trouble for {FACTION}, so they've chosen a champion in {METROSCENE} to challenge you in single mecha combat.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self.activate_mission,
                data={"subject": "this challenge"}, subject="{FACTION} have challenged".format(**self.elements),
                no_repeats=True
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if not self.mission_active and npc not in camp.party:
            ...
            mygram["[HELLO_AGAIN]"].append(
                "What are you doing here, [audience]?! Didn't you hear that {FACTION} have challenged you to a one-on-one duel against their champion?".format(
                    **self.elements))
            mygram["[HELLO_FIRST]"].append(
                "You're [audience], aren't you? [chat_lead_in] {FACTION} have challenged you to a one-on-one duel against their champion.".format(
                    **self.elements)
            )
        return mygram
...
class DZREPRC_RazeTheFortress(DZREPRC_ConclusionTemplate):
    ...
    def NPC_offers(self, camp):
        ...
        if not self.mission_active:
            mylist.append(Offer(
                "[audience], I was hoping to find you. We have finally discovered the location of {FACTION}'s base.".format(
                    **self.elements),
                context=ContextTag((context.HELLO,)), no_repeats=True
                ))
            mylist.append(Offer(
                "You are authorized to attack immediately. If we can disable their base, we can stop their operation dead in its tracks. [IWillSendMissionDetails]".format(
                    **self.elements),
                context=ContextTag((context.CUSTOM,)), effect=self.activate_mission,
...
class DZREPR_PrettyStandardBandits(DZREPR_BasePlot):
    ...
    def _get_generic_offers(self, npc, camp):
        """Get any offers that could apply to non-element NPCs."""
        goffs = list()
        if not self.start_missions and npc not in camp.party and not camp.are_faction_allies(npc,
                                                                                             self.elements["FACTION"]):
            goffs.append(Offer(
                msg="[THEYARETHIEVES] They work the highway between here and {}, taking what they can from who they can.".format(
                    self.elements["DZ_EDGE"].get_city_link(self.elements["METROSCENE"])),
    ...
    def _get_briefed(self, camp):
        ...
        camp.check_trigger("UPDATE")
        self.memo = "You learned that {} have been robbing travelers near {}.".format(self.elements["FACTION"],
                                                                                      self.elements["METROSCENE"])
...
class DZREPR_HiddenBandits(DZREPR_BasePlot):
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if npc not in camp.party and not self.start_missions:
            mygram["[News]"].append(
                "nobody knows much about {FACTION}, except that they can seemingly strike from nowhere ".format(
                    **self.elements))
        return mygram
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.start_missions and npc not in camp.party and not camp.are_faction_allies(npc,
                                                                                             self.elements["FACTION"]):
            goffs.append(Offer(
                msg="[THEYAREAMYSTERY] They've been raiding between here and {} but nobody knows where their base is.".format(
                    self.elements["DZ_EDGE"].get_city_link(self.elements["METROSCENE"])),
                context=ContextTag((context.INFO,)), effect=self._get_briefed,
                subject=str(self.elements["FACTION"]),
                data={"subject": str(self.elements["FACTION"]), "they": str(self.elements["FACTION"])}, no_repeats=True
    ...
    def _get_briefed(self, camp):
        self.start_missions = True
        camp.check_trigger("UPDATE")
        self.memo = "You learned that {} must have have a hidden base near {}.".format(self.elements["FACTION"],
                                                                                       self.elements["METROSCENE"])
...
class DZREPR_ToleratedBandits(DZREPR_BasePlot):
    ...
    def _get_dialogue_grammar(self, npc, camp):
        mygram = collections.defaultdict(list)
        if npc not in camp.party and not self.start_missions:
            mygram["[News]"].append("{} have been raiding convoys between here and {}".format(self.elements["FACTION"],
                                                                                              self.elements[
                                                                                                  "DZ_EDGE"].get_city_link(
                                                                                                  self.elements[
                                                                                                      "METROSCENE"])))
        return mygram
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.start_missions and npc not in camp.party and not camp.are_faction_allies(npc,
                                                                                             self.elements["FACTION"]):
            goffs.append(Offer(
                msg="The dead zone has always been full of bandits; {FACTION} may be a big deal now, but in time they will fade away or get taken out by a bigger gang.".format(
                    **self.elements),
                context=ContextTag((context.INFO,)), effect=self._get_briefed,
                subject=str(self.elements["FACTION"]),
                data={"subject": str(self.elements["FACTION"]), "they": str(self.elements["FACTION"])}, no_repeats=True
    ...
    def _get_briefed(self, camp):
        self.start_missions = True
        camp.check_trigger("UPDATE")
        self.memo = "You learned that {} have been raiding convoys near {}.".format(self.elements["FACTION"],
                                                                                    self.elements["METROSCENE"])
...
class DZREPR_DislikedBandits(DZREPR_BasePlot):
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if npc not in camp.party and not self.start_missions:
            mygram["[News]"].append(
                "{FACTION} have been causing a lot of problems in {METROSCENE}".format(**self.elements))
        return mygram
    ...
    def _get_generic_offers(self, npc, camp):
        ...
        if not self.start_missions and npc not in camp.party and not camp.are_faction_allies(npc,
                                                                                             self.elements["FACTION"]):
            goffs.append(Offer(
                msg="[THEYAREOURENEMY] It's because of their raids that we can't trade freely with {}.".format(
                    self.elements["DZ_EDGE"].get_city_link(self.elements["METROSCENE"])),
                context=ContextTag((context.INFO,)), effect=self._get_briefed,
                subject=str(self.elements["FACTION"]),
                data={"subject": str(self.elements["FACTION"]), "they": str(self.elements["FACTION"])}, no_repeats=True
    ...
    def _get_briefed(self, camp):
        self.start_missions = True
        camp.check_trigger("UPDATE")
        self.memo = "You learned that {} have been causing trouble in {}.".format(self.elements["FACTION"],
                                                                                  self.elements["METROSCENE"])
...
class DZREPR_NewBanditsWhoThis(DZREPR_BasePlot):
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if camp.scene.get_root_scene() is self.elements["METROSCENE"] and npc is not self.elements[
            ...
            mygram["[News]"].append("the trucker {} lost {} convoy on the way into town.".format(self.elements["NPC"],
                                                                                                 self.elements[
                                                                                                     "NPC"].gender.possessive_determiner))
        if camp.scene.get_root_scene() is self.elements["METROSCENE"] and not self.start_missions:
            mygram["[News]"].append("there never used to be bandits between here and {} in the past".format(
                self.elements["DZ_EDGE"].get_city_link(self.elements["METROSCENE"])))
        return mygram
    ...
    def _get_generic_offers(self, npc, camp):
        """Get any offers that could apply to non-element NPCs."""
        goffs = list()
        if not self.got_rumor and camp.scene.get_root_scene() is self.elements["METROSCENE"] and npc is not \
                self.elements["NPC"]:
            mynpc = self.elements["NPC"]
            goffs.append(Offer(
                msg="I think you should speak to {} directly about all this; you can probably find {} at {}.".format(
                    mynpc, mynpc.gender.object_pronoun, self.elements["_DEST"]),
                context=ContextTag((context.INFO,)), effect=self._get_rumor,
                subject=str(mynpc), data={"subject": str(mynpc)}, no_repeats=True
            ))
        if not self.start_missions and npc is not self.elements["NPC"] and npc not in camp.party:
            goffs.append(Offer(
                msg="[as_far_as_I_know] they call themselves {}. Hopefully they will leave this place as quickly as they arrived.".format(
                    self.elements["FACTION"]),
                context=ContextTag((context.INFO,)), effect=self._get_rumor,
    ...
    def NPC_offers(self, camp):
        ...
        if not self.start_missions:
            mylist.append(Offer(
                "[HELLO] While bringing in a convoy from {}, I got attacked by bandits and lost everything.".format(
                    self.elements["DZ_EDGE"].get_city_link(self.elements["METROSCENE"])),
                context=ContextTag([context.HELLO]),
                ))

            mylist.append(Offer(
                "I can tell you that {} haven't been around for long- if I knew there were raiders working this stretch I would have brought along more muscle. If you can find their base it shouldn't be hard to get rid of them.".format(
                    self.elements["FACTION"]),
                context=ContextTag([context.INFO]), effect=self._get_briefed,
                data={"subject": "the bandits"}, no_repeats=True
                ))

        elif self.elements[E_MISSION_NUMBER] > 1 and self.elements["NPC"].get_reaction_score(camp.pc,
                                                                                             camp) > 0 and not self.offered_services:
            mylist.append(Offer(
                "[HELLO] Now that my mek's repaired, I'd love a chance to take on {} myself.".format(
                    self.elements["FACTION"]),
                context=ContextTag([context.HELLO]),
            ))
            if camp.can_add_lancemate():
                ...
                mylist.append(Offer(
                    "Seems like it'd be a little crowded to join your [lance] now, but if you need my help in the future come back and find me.",
    ...
    def _get_briefed(self, camp):
        self.start_missions = True
        camp.check_trigger("UPDATE")
        self.memo = "You learned that {} must have a base near {}.".format(self.elements["FACTION"],
                                                                           self.elements["METROSCENE"])
...
class DZREPR_PrettyStandardInvaders(DZREPR_BasePlot):
    ...
    def _get_dialogue_grammar(self, npc, camp):
        ...
        if npc not in camp.party and not self.start_missions:
            mygram["[News]"].append(
                "{FACTION} have been trying to establish a stronghold nearby".format(**self.elements))
        return mygram
    ...
    def _get_briefed(self, camp):
        self.start_missions = True
        camp.check_trigger("UPDATE")
        self.memo = "You learned that {} are trying to establish a stronghold near {}.".format(self.elements["FACTION"],
                                                                                               self.elements[
                                                                                                   "METROSCENE"])
